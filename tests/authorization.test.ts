...
test('authorize client, should throw error: "invalid_code"', async () => {
  try {
    ...
    expect(value.error_description).toBe('Invalid authorization code: invalid_code')
  }
})
...
test('authorize client, should throw error: "missing_access_token"', async () => {
  try {
    ...
    expect(value.error_description).toBe('Access token is missing or not stored in the app.')
  }
})
